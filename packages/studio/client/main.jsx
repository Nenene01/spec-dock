...
function Tabs({ pane, open, close, beginDrag, endDrag }) { return <div className="editor-tabs">{pane.tabs.map((tab) => <div className={tab === pane.active ? "editor-tab active" : "editor-tab"} draggable onDragStart={(event) => beginDrag(event, tab)} onDragEnd={endDrag} key={tab}><button title={titleOf(tab)} onClick={() => open(tab)}>{titleOf(tab)}</button><button className="close-tab" aria-label={titleOf(tab) + "を閉じる"} onClick={(event) => { event.stopPropagation(); close(tab); }}>×</button></div>)}</div>; }
function App() {
