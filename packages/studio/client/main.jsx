...
function schemaRows(schema, prefix = "") { if (!schema) return []; const rows = []; const add = (value, name, required, path) => { const currentPath = path ? `${path}.${name}` : name; rows.push(<tr key={currentPath}><td><code>{currentPath}</code></td><td><code>{schemaType(value)}</code></td><td>{required ? "必須" : "任意"}</td><td>{schemaConstraint(value)}</td><td>{value.description || ""}</td></tr>); if (value.properties) rows.push(...schemaRows(value, currentPath)); if (value.items?.properties) rows.push(...schemaRows(value.items, `${currentPath}[]`)); }; if (schema.allOf) schema.allOf.forEach((part) => rows.push(...schemaRows(part, prefix))); Object.entries(schema.properties || {}).forEach(([name, value]) => add(value, name, schema.required?.includes(name), prefix)); return rows; }
function SchemaTable({ schema }) { return <Table headers={["項目", "型", "必須", "制約", "説明"]} rows={schemaRows(schema)} empty="スキーマ定義はありません。"/>; }
...
function ApiPage({ id }) { if (id == null) return <><Head eyebrow="INTERFACE" title="API一覧" description="APIの入出力と制約を確認します。" /><p className="empty">サイドバーからAPIを選択してください。</p></>; const item = model.openapi.operations[Number(id)]; const responseHeaders = (item.responses || []).flatMap((response) => (response.headers || []).map((header) => ({ ...header, status: response.status }))); return <><Head eyebrow="API DETAIL" title={`${item.method} ${item.path}`} description={item.summary || item.operationId} /><section className="panel api-contract"><h2>API概要</h2><div className="contract-grid"><div><span>認証方式</span><strong>{securityLabel(item.security)}</strong></div><div><span>サーバー</span><strong>{item.servers?.[0]?.url || "定義なし"}</strong></div><div><span>操作ID</span><strong>{item.operationId || "定義なし"}</strong></div></div></section><section className="panel"><h2>パラメータ / ヘッダー</h2><Table headers={["名前", "場所", "型", "必須", "説明"]} rows={(item.parameters || []).map((p) => <tr key={`${p.in}-${p.name}`}><td><code>{p.name}</code></td><td>{p.in}</td><td>{p.schema?.type || p.schema?.ref || ""}</td><td>{p.required ? "必須" : "任意"}</td><td>{p.description || ""}</td></tr>)} empty="パラメータとヘッダーはありません。" /></section>{item.requestBody && <section className="panel request-body"><div className="section-heading"><h2>リクエストボディ</h2><span className={item.requestBody.required ? "required-badge" : "optional-badge"}>{item.requestBody.required ? "必須" : "任意"}</span></div><div className="schema-meta">{(item.requestBody.contentTypes || [item.requestBody.contentType || "application/json"]).map((type) => <code key={type}>{type}</code>)}{item.requestBody.schema?.ref && <code>{item.requestBody.schema.ref}</code>}</div>{item.requestBody.description && <p className="muted">{item.requestBody.description}</p>}<SchemaTable schema={item.requestBody.schema}/></section>}<section className="panel"><h2>レスポンス</h2><Table headers={["Status", "説明", "Content-Type", "スキーマ"]} rows={(item.responses || []).map((r) => <tr key={r.status}><td>{r.status}</td><td>{r.description}</td><td>{(r.contentTypes || []).join(", ") || "—"}</td><td>{r.schema?.ref || r.schema?.type || "—"}</td></tr>)}/>{responseHeaders.length > 0 && <><h3>Response headers</h3><Table headers={["Status", "Header", "型", "必須", "説明"]} rows={responseHeaders.map((header) => <tr key={`${header.status}-${header.name}`}><td>{header.status}</td><td><code>{header.name}</code></td><td>{header.schema?.type || header.schema?.ref || ""}</td><td>{header.required ? "必須" : "任意"}</td><td>{header.description || ""}</td></tr>)}/></>}</section></>; }
function DatabasePage({ id }) { if (id == null) return <><Head eyebrow="DATA MODEL" title="データモデル" description="Prismaで定義されたモデル、インデックス、制約、リレーションを確認します。" /><p className="empty">サイドバーからモデルを選択してください。</p></>; const item = model.prisma.models[Number(id)]; return <><Head eyebrow="DATA MODEL DETAIL" title={item.name} description={item.description} /><section className="panel"><h2>フィールド</h2><Table headers={["名前", "型", "属性", "説明"]} rows={item.fields.map((f) => <tr key={f.name}><td><code>{f.name}</code></td><td>{f.type}{f.isArray ? "[]" : ""}{f.isOptional ? "?" : ""}</td><td>{[f.isId && "PK", f.isUnique && "UK", f.dbType].filter(Boolean).join(" / ")}</td><td>{f.description}</td></tr>)}/></section><section className="panel"><h2>インデックス・制約</h2><Table headers={["種類", "フィールド", "名前"]} rows={(item.constraints || []).map((c) => <tr key={c.name || c.fields.join(",")}><td>{c.kind}</td><td>{c.fields.join(", ")}</td><td>{c.name || ""}</td></tr>)} empty="モデル制約はありません。"/></section></>; }
...
function App() {
  ...
  const renderPage = (target, paneIndex) => { const { view, id } = parseRoute(target); const paneOpen = (next) => open(next, paneIndex); if (view === "overview") return <Overview open={paneOpen}/>; if (view === "api") return <ApiPage id={id}/>; if (view === "database") return <DatabasePage id={id}/>; if (view === "zod") return <ZodPage id={id}/>; if (view === "document") return <DocumentPage id={id} open={paneOpen}/>; if (view === "er") return <ErPage/>; return <Overview open={paneOpen}/>; };
